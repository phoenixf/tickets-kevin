...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <div class="space-y-4 mb-6">
                ...
                <?php else : ?>
                    <?php foreach ($comentarios as $comentario) : ?>
                        <div class="border-l-4 <?= $comentario['eh_interno'] ? 'border-yellow-400 bg-yellow-50' : 'border-gray-300 bg-gray-50' ?> p-4 rounded">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    <span class="font-semibold text-gray-900"><?= esc($comentario['usuario_nome']) ?></span>
                            ...
                            <p class="text-gray-700 whitespace-pre-wrap"><?= esc($comentario['conteudo']) ?></p>
                        </div>
                    <?php endforeach ?>
                <?php endif ?>
            </div>
            ...
            <form action="/tickets/<?= $ticket['id'] ?>/comments" method="POST" class="border-t pt-4">
                <?= csrf_field() ?>
                ...
                <?php if ($user->funcao !== 'cliente') : ?>
                    <div class="mb-3">
                        <label class="flex items-center">
                            <input type="checkbox" name="eh_interno" value="1" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            <span class="ml-2 text-sm text-gray-700">Comentário interno (não visível para clientes)</span>
                        </label>
                    </div>
                <?php endif ?>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <div class="space-y-2 mb-6">
                <?php if (empty($anexos)) : ?>
                    <p class="text-gray-500 text-sm">Nenhum anexo.</p>
                <?php else : ?>
                    <?php foreach ($anexos as $anexo) : ?>
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-gray-900"><?= esc($anexo['nome_arquivo']) ?></p>
            ...
            <form action="/tickets/<?= $ticket['id'] ?>/attachments" method="POST" enctype="multipart/form-data" class="border-t pt-4">
                <?= csrf_field() ?>
